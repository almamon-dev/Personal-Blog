...
export default function Welcome({ auth, blogs, currentCategory }) {
    ...
    const mockToken = "a" + Array.from({ length: 254 }, () => Math.random().toString(36).charAt(2)).join("");
                        ...
                        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-6 border-b border-gray-200 pb-8">
                            <div>
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Technical Tutorials</h2>
                                </div>
                                
                                {/* Category Filter Tabs */}
                                <div className="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
                                    <Link 
                                        href={route('welcome', { token: mockToken })}
                                        preserveScroll
                                    ...
                                    {globalCategories && globalCategories.map((cat) => (
                                        <Link 
                                            key={cat.id}
                                            href={route('welcome', { category: cat.name, token: mockToken })}
                                            preserveScroll
                            ...
                            >
                                Explore All Content <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                            </Link>
                        </div>
                                    ...
                                    <div className="p-6 flex-1 flex flex-col">
                                        ...
                                        <div className="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-bold text-gray-700">Savanihd</span>
                                            </div>
